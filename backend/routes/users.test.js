...
describe("POST /users", function () {
  ...
  test("doesnt work for non-admins: create admin", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
...
describe("GET /users", function () {
  ...
  test("doesnt work for non-admins", async function () {
    const resp = await request(app)
        .get("/users")
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("PATCH /users/:username", () => {
  ...
  test("doesnt work for non-admin non self", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          firstName: "New",
  ...
  test("works for admin AND self", async function () {
    const resp = await request(app)
        .patch(`/users/u3`)
        .send({
          firstName: "New",
